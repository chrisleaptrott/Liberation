// escortHeli.sqf
// © OCTOBER 2011 - norrin
private ["_escort","_heli","_base","_group","_groupH","_slickDisabled","_unit","_pos","_offset","_dir","_targets"];
...
{	
	...
	[_heli,objNull] spawn Nor_HT_SS1;
	_groupH setFormation "Line";
	// Wait until slick gets to destination
	while {([_escort,_heli] call Nor_HTE_C3) && ((getPos _heli) select 2) > 15 && (_escort distance _pos) > 50} do {_escort commandMove _offset; sleep 5};
	if ([_escort,_heli] call Nor_HTE_C3 &&(_heli getVariable "NORRN_H_destChosen")) then 
	{
		...
		_targets = (vehicle _unit) nearTargets 300;
